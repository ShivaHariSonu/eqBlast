...
parser.add_argument(
    "--dir",
    type=str,
    default="trainBaseSWAG",
    required=True,
    help="training directory (default: None)",
)

parser.add_argument(
    "--dataset", type=str, default="custom", help="dataset name (default: CIFAR10)"
)
parser.add_argument(
    "--training_data_path",
    type=str,
...
parser.add_argument(
    ...
    default=None,
    metavar="CKPT",
    help="checkpoint to restore SWA from (default: None)",
)
parser.add_argument(
    "--loss",
    type=str,
    default="BCE_logit",
    help="loss to use for training model (default: Binary Cross-entropy Logit loss)",
)
...
logging.info("Loading dataset %s from %s" % (args.dataset, args.training_data_path))
loaders, num_classes = data.loaders(
    ...
    test_file = args.validation_data_path
)
...
elif args.loss == "BCE_logit":
    criterion = losses.binary_cross_entropy_logits
...
columns = ["ep", "lr", "tr_loss", "tr_acc", "ve_loss", "ve_acc", "time", "mem_usage"]
if args.swa:
    columns = columns[:-2] + ["swa_ve_loss", "swa_ve_acc"] + columns[-2:]
...
for epoch in range(start_epoch, args.epochs):
    ...
    if (
        ...
        training_acc_scores.append(train_res["accuracy"])
        validation_acc_scores.append(test_res["accuracy"])
...
if args.swa:
    np.savez(
        ...
        targets=sgd_targets,
        training_acc_scores = training_acc_scores,
        validation_acc_scores = validation_acc_scores,
