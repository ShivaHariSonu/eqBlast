...
def train_epoch(
    ...
    for i, (input, target) in enumerate(loader):
        ...
        if not regression:
            sigmoid_op = torch.sigmoid(output.data)
            pred = torch.where(sigmoid_op>threshold,torch.tensor(1),torch.tensor(0))
            target_vector = target
            correct += pred.eq(target_vector.data.view_as(pred)).sum().item()
...
def eval(loader, model, criterion, cuda=True, regression=False, verbose=False):
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(loader):
            ...
            if not regression:
                pred = torch.where(output.data>threshold,torch.tensor(1),torch.tensor(0))
                target_vector = target
                correct += pred.eq(target_vector.data.view_as(pred)).sum().item()
