    ...
    **kwargs,
    ...
    if dataset =="custom":
        ...
        if (not os.path.exists(test_file)):
            print("Validation file {} does not exist".format(test_file))
        ...
        if (os.path.exists(test_file)):
            print("Loading data from validation file {}".format(test_file))
            with h5py.File(test_file, 'r') as f:
                X_test = f['X'][:]
                y_test = f['y'][:]
                assert len(X_test) == len(y_test)
            print("Number of validation dataset examples {}".format(len(X_test)))
        ...
        if (not os.path.exists(training_file)):
            sys.exit("Training file {} does not exist".format(training_file))
        if (not os.path.exists(test_file)):
            print("Test file {} does not exist".format(test_file))
        if (os.path.exists(training_file)):
            with h5py.File(training_file, 'r') as f:
                X_train = f['X'][:]
                y_train = f['y'][:]
            ...
            training_dataset = NumpyDataset(X_train, y_train)
        if (os.path.exists(test_file)):
            print("Loading data from test file {}".format(test_file))
            with h5py.File(test_file, 'r') as f:
                X_test = f['X'][:]
                y_test = f['y'][:]
                assert len(X_test) == len(y_test)
            print("Number of test dataset examples {}".format(len(X_test)))
            test_dataset = NumpyDataset(X_test, y_test)
        return (
            {
                "train": torch.utils.data.DataLoader(
